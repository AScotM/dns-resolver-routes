...
class DNSRecord:
    ...
    def __str__(self):
        ...
        ttl_display = self.remaining_ttl if self.cache_time else self.ttl
        
        if self.type == DNSRecordType.MX:
            return f"{type_str}\t{ttl_display}\t{self.preference}\t{self.data}"
        elif self.type == DNSRecordType.SOA:
            soa = self.data
            return (f"{type_str}\t{ttl_display}\t{soa['mname']} {soa['rname']} "
                    f"({soa['serial']} {soa['refresh']} {soa['retry']} "
                    f"{soa['expire']} {soa['minimum']})")
        elif self.type == DNSRecordType.TXT:
            return f"{type_str}\t{ttl_display}\t\"{self.data}\""
        elif self.type == DNSRecordType.SRV:
            srv = self.data
            return f"{type_str}\t{ttl_display}\t{srv['priority']} {srv['weight']} {srv['port']} {srv['target']}"
        return f"{type_str}\t{ttl_display}\t{self.data}"
...
class DNSCache:
    def __init__(self, max_size: int = 1024):
        ...
        self._cache: Dict[str, Tuple[float, List[DNSRecord]]] = OrderedDict()
        self._lock = threading.RLock()
    
    def get(self, key: str) -> Optional[List[DNSRecord]]:
        with self._lock:
            if key not in self._cache:
                return None
            
            timestamp, records = self._cache[key]
            ...
            for record in records:
                ...
                if record.ttl > elapsed:
                    remaining_ttl = record.ttl - int(elapsed)
                    ...
                    valid_records.append(cached_record)
    ...
    def put(self, key: str, records: List[DNSRecord]):
        with self._lock:
...
class DNSResolver:
    ...
    def __init__(
        ...
        self._tcp_socket_pool: OrderedDict[Tuple[str, int], socket.socket] = OrderedDict()
        self._tcp_pool_lock = threading.Lock()
        self._udp_sockets: Dict[Tuple[str, int, int], socket.socket] = {}
        self._udp_socket_lock = threading.Lock()
        self._cache = DNSCache(max_cache_size)
        self._rate_limit_lock = threading.Lock()
        self._last_query_time: Dict[Tuple[str, int], float] = {}
    ...
    def _build_query(
        ...
        return header + question + edns, tid, source_port
    ...
    def _cleanup_tcp_pool(self):
        with self._tcp_pool_lock:
            if len(self._tcp_socket_pool) > self.max_tcp_connections:
                excess = len(self._tcp_socket_pool) - self.max_tcp_connections
                for _ in range(excess):
                    server, sock = self._tcp_socket_pool.popitem(last=False)
                    try:
                        sock.close()
                    except Exception:
                        pass
    ...
    def _get_tcp_socket(self, server: Tuple[str, int]) -> socket.socket:
        with self._tcp_pool_lock:
            if server in self._tcp_socket_pool:
                sock = self._tcp_socket_pool[server]
                self._tcp_socket_pool.move_to_end(server)
                try:
                    ...
                    try:
                        ...
                        pass
                    self._tcp_socket_pool.pop(server, None)
            ...
            for family, socktype, proto, canonname, sockaddr in addr_info:
                try:
                    ...
                    self._tcp_socket_pool[server] = sock
                    return sock
                except Exception:
                    if sock:
                        try:
    ...
    def _get_udp_socket(self, server: Tuple[str, int], family: int) -> socket.socket:
        key = (server[0], server[1], family)
        
        with self._udp_socket_lock:
            if key in self._udp_sockets:
                return self._udp_sockets[key]
            ...
            try:
                ...
                raise socket.error(f"Could not create UDP socket: {e}")
        ...
        while len(data) < n:
            try:
                ...
                if time.time() - start_time > self.timeout:
                    raise socket.error("Timeout while receiving data")
                continue
        
        return bytes(data)

    def _apply_rate_limit(self, server: Tuple[str, int]):
        with self._rate_limit_lock:
            current_time = time.time()
            last_time = self._last_query_time.get(server, 0)
            
            if current_time - last_time < self.RATE_LIMIT_DELAY:
                time.sleep(self.RATE_LIMIT_DELAY - (current_time - last_time))
            
            self._last_query_time[server] = time.time()

    def _check_pending_limit(self):
        with self._pending_lock:
            if self._pending_queries >= self.MAX_PENDING_QUERIES:
                raise RuntimeError(f"Too many pending queries (max {self.MAX_PENDING_QUERIES})")
    ...
    def _send_query(self, query: bytes, server: Tuple[str, int], source_port: int = None) -> bytes:
        ...
        try:
            if self.use_tcp:
                ...
                for family, socktype, proto, canonname, sockaddr in addr_info:
                    try:
                        sock = self._get_udp_socket(server, family)
                        ...
                        data, addr = sock.recvfrom(self.MAX_UDP_SIZE)
        ...
        for attempt in range(self.retries):
            for current_server in servers:
                try:
                    ...
                    query, tid, source_port = self._build_query(domain, query_type, self.request_dnssec)
                    start_time = time.time()

                    data = self._send_query(query, current_server, source_port)
                    if not data:
    ...
    def close(self):
        with self._tcp_pool_lock:
            for server, sock in list(self._tcp_socket_pool.items()):
                try:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="async_mode",
        action="store_true",
        help="Enable async mode for concurrent queries"
    )
    ...
    try:
        ...
        with DNSResolver(
            use_tcp=args.tcp,
            request_dnssec=args.request_dnssec,
